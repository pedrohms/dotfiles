...
if [[ -n $IS_HDMI ]]; then
  if [[ $PH_NVIDIA -eq "2" ]]; then
    ...
    xrandr --output HDMI-1-0 --mode 1920x1080 --dpi 96 --rate 60 --pos 1920x0 --output eDP-1-1 --primary --mode 1920x1080 --rate 120 --pos 0x0
  else
    xrandr --output HDMI-0 --off
    xrandr --output HDMI-0 --mode 1920x1080 --dpi 96 --rate 60 --pos 1920x0 --output eDP-1-1 --primary --mode 1920x1080 --rate 120 --pos 0x0
  fi
else 
  if [[ -n $IS_DP_1 ]]; then
    if [[ $PH_NVIDIA -eq "2" ]]; then
