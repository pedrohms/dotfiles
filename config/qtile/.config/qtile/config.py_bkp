...
colors = []
cache='/home/pedro/.cache/wal/colors'
...
def init_widgets_list(monitor_num):
    widgets_list = [
        ...
        widget.TextBox(text = "󰕾 ", fontsize = 14, font = "JetBrainsMono Nerd Font", foreground = colors[6]),
        widget.PulseVolume(
                    foreground=colors[8],
                    padding=10,
                ),
        ...
        widget.Sep(linewidth = 1, padding = 10, foreground = colors[0], background = colors[0]),
        widget.CurrentLayoutIcon(scale = 0.5, foreground = colors[0], background = colors[0], padding = 10,),
    ]
...
widgets_list.insert(widgets_list.count(widgets_list)-2, widget.Systray(background = colors[0], icon_size = 20, padding = 4))

secondary_widgets_list = init_secondary_widgets_list("2")

screens = [
    Screen(top=bar.Bar(widgets=widgets_list, size=36, background=colors[0], margin=6, opacity=0.8),),
