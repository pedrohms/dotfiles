...
myStartupHook = do
  ...
  spawnOnce "cpupower-gui -p &"
  spawnOnce "xrandr --output eDP1 --primary --auto --output HDMI1 --right-of eDP1 --auto"
  spawnOnce "blueman-applet &"
  spawnOnce "rclone --vfs-cache-mode writes mount onedrive: ~/cloud/onedrive/ &"
  spawnOnce "rclone --vfs-cache-mode writes mount googledrive: ~/cloud/googledrive/ &"
...
myScratchPads = [ NS "terminal" spawnTerm findTerm manageTerm
                , NS "notes" spawnStickyNote findStickyNote manageStickyNote
                , NS "mocp" spawnMocp findMocp manageMocp
                , NS "calculator" spawnCalc findCalc manageCalc
                ]
  where
    spawnTerm = myTerminal ++ " -t scratchpad -e tmux" -- myTerminal ++ " -n scratchpad"
    ...
    manageTerm = customFloating $ W.RationalRect l t w h
      where
        ...
        t = 0.95 - h
        l = 0.95 - w
    spawnStickyNote = myTerminal ++ " -t scratch-notes -e nvim ~/personal/notes.md"
    findStickyNote = title =? "scratch-notes"
    manageStickyNote = customFloating $ W.RationalRect l t w h
      where
        h = 0.9
        w = 0.4
        t = 0.04
...
myKeys c =
  ...
  , ("M-C-f", addName "Launch pcmanfm"         $ spawn (myFileManager))
  ...
  , ("M-C-n", addName "Toggle scratchpad NOTE.md"   $ namedScratchpadAction myScratchPads "notes")
  ]
  ...
  , ("<XF86Eject>", addName "Eject /dev/cdrom"        $ spawn "eject /dev/cdrom")
    -- ScreenShot
  , ("<Print>", addName "ScreenShot - Monitor" $ spawn "scrot ~/scrot/%Y-%m-%d-@%H-%M-%S-scrot.png")
  , ("M-C-s w", addName "ScreenShot - Window" $ spawn "scrot -u ~/scrot/%Y-%m-%d-@%H-%M-%S-scrot.png")
  , ("M-C-s s", addName "ScreenShot - Slice" $ spawn "scrot -s ~/scrot/%Y-%m-%d-@%H-%M-%S-scrot.png")
    -- Touchpad
  , ("M-C-t d", addName "Touchpad Lenovo - Disable" $ spawn "xinput disable 'Synaptics TM3336-004'")
  , ("M-C-t e", addName "Touchpad Lenovo - Enable" $ spawn "xinput enable 'Synaptics TM3336-004'")
    -- Dev
  , ("M-C-d a", addName "Android-studio" $ spawn "android-studio") -- android-studio
  , ("M-C-d c", addName "Visual Studio Code" $ spawn "code") -- vscode
  , ("M-C-d n", addName "Neovide - GUI Vim" $ spawn "neovide")
    -- cpupower profiles
  ,("M-M1-C-p p", addName "CPUPower - Performance" $ spawn "cpupower-gui -p")
  ,("M-M1-C-p b", addName "CPUPower - Balance" $ spawn "cpupower-gui -b")
   -- keyboard locale
  ,("M-S-k b", addName "Keyboard - br mapping" $ spawn "/bin/bash -c 'setxkbmap br'")
  ,("M-S-k u", addName "Keyboard - us mapping" $ spawn "/bin/bash -c 'setxkbmap us")
   -- Applications
  ,("M-C-a d", addName "Discord" $ spawn "disc") -- discord
  ,("M-C-a p", addName "CPUPower - Gui" $ spawn "cpupower-gui") -- cpupower-gui
  ,("M-C-a c", addName "CFFClient" $ spawn "LANG=\'pt_BR.UTF-8\' wine ~/.wine/drive_c/Compu\\ Forte/CFFClient/CFFClient.exe")
  ,("M-C-a t", addName "CompuTM" $ spawn "LANG=\'pt_BR.UTF-8\' wine ~/.wine/drive_c/Compu\\ Forte/CompuTM/CompuTM.exe")
  ,("M-C-a u", addName "CompuTouch" $ spawn "LANG=\'pt_BR.UTF-8\' wine ~/.wine/drive_c/Compu\\ Forte/CompuTouch/CompuTouch.exe")
  ,("M-C-a f", addName "Ranger" $ spawn "ranger")
  ]
